...
<body class="bg-light">
  <div class="container mt-5">
    ...
    <form method="POST" class="card p-4 shadow-sm bg-white">
      ...
      <a href="officials/official_dashboard.php" class="btn btn-secondary">Back to Dashboard</a>
    </form>
    ...
    <ul class="list-group">
      <?php
      $stmt = $conn->query("SELECT category_name, priority_level FROM categories ORDER BY category_name ASC");
      while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
          echo "<li class='list-group-item d-flex justify-content-between align-items-center'>
                  {$row['category_name']}
                  <span class='badge bg-info'>{$row['priority_level']}</span>
                </li>";
      }
      ?>
    </ul>
